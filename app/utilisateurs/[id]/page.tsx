...
export default function UtilisateurDetailPage() {
  ...
  // Dupliquées ici pour validation live (UX) ; la source de vérité reste l'API.
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        // L'API renvoie issues.password = string[] avec un message par règle violée.
        const issues = (err.issues as { password?: string[] } | undefined)?.password;
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl p-6 w-full max-w-md space-y-4">
            ...
            />
            {/* Checklist live des règles serveur — chaque ligne devient verte quand satisfaite */}
