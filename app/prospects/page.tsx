...
import {
  UserPlus, Plus, Search, LayoutGrid, List, Building2, User,
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        <StatCard label="Total" value={besoins.length} icon={UserPlus} color="text-gray-400 bg-gray-500/10" />
      ...
      {loading ? (
        ...
        <KanbanView cols={PIPELINE_COLS} byStatut={byStatut} onOpen={(id) => router.push(`/prospects/${id}`)} />
      ) : (
        <ListView besoins={filtered} onOpen={(id) => router.push(`/prospects/${id}`)} />
      )}
    </div>
  );
}
