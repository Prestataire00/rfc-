...
export default function EspaceClientPage() {
  ...
  const cards = [
    { label: "Salaries / Stagiaires", value: stats?.nbStagiaires || 0, icon: Users, color: "bg-red-900/200", href: "/espace-client/stagiaires" },
    { label: "Sessions a venir", value: stats?.nbSessionsAVenir || 0, icon: CalendarDays, color: "bg-amber-900/200", href: "/espace-client/formations" },
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-100">Espace Client</h1>
        <p className="text-gray-400 mt-1">Suivez les formations de vos collaborateurs</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((c) => (
          <Link key={c.label} href={c.href} className="block rounded-lg border bg-gray-800 p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-400">{c.label}</p>
                <p className="text-2xl font-bold text-gray-100 mt-1">{c.value}</p>
              </div>
