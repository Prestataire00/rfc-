...
const STATUT_COLORS: Record<string, string> = {
  a_payer: "bg-amber-500/20 text-amber-700 dark:text-amber-300",
  paye: "bg-emerald-500/20 text-emerald-700 dark:text-emerald-300",
  refuse: "bg-red-600/30 text-red-700 dark:text-red-300",
};
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
          <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">A payer</p>
          <p className="text-2xl font-bold text-amber-400 mt-1">{formatCurrency(totalAPayer)}</p>
        </div>
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
          <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">Nb factures</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">{filtered.length}</p>
        </div>
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
          <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">Cette periode</p>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400">Numero</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400">Formateur</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400">Session</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400">Date prestation</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400">Montant TTC</th>
              ...
              {filtered.map((f) => (
                <tr key={f.id} className="hover:bg-gray-750 cursor-pointer">
                  ...
                  <td className="px-4 py-3 text-xs">
                    {f.session ? (
                      <Link href={`/sessions/${f.session.id}`} className="text-gray-500 dark:text-gray-400 hover:text-red-400">
                  ...
                  <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{formatDate(f.datePrestation)}</td>
          ...
          <div className="space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            {form.montantHT && (
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Montant TTC : <span className="font-semibold text-gray-900 dark:text-gray-100">
