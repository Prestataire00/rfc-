    ...
    <div>
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={`rounded-full px-3 py-1 text-xs font-medium border ${!filterType ? "bg-blue-100 text-blue-700 border-blue-300" : "bg-gray-50 text-gray-600 border-gray-200"}`}
        >
          Tous
        </button>
        {Object.entries(TYPE_LABELS).map(([key, label]) => (
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-medium border ${filterType === key ? "bg-blue-100 text-blue-700 border-blue-300" : "bg-gray-50 text-gray-600 border-gray-200"}`}
          >
            {label}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((doc) => (
            <div key={doc.id} className="rounded-lg border bg-white p-4 hover:shadow-md transition-shadow">
