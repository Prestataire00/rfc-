...
export async function POST(req: NextRequest) {
  ...
  if (!process.env.SMTP_USER) {
    return NextResponse.json({
      success: false,
      message: "SMTP non configure. Ajoutez SMTP_USER et SMTP_PASS dans .env",
    });
  }
  ...
  try {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 24px; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 8px 8px;">
            ...
            <p style="color: #22c55e; font-weight: bold;">La configuration est operationnelle !</p>
            <p style="color: #64748b; font-size: 12px; margin-top: 24px;">
              Envoye depuis FormaPro CRM
            </p>
          </div>
        </div>
      `,
    ...
    return NextResponse.json({ success: true, message: "Email envoye avec succes !" });
  } catch (error: any) {
    return NextResponse.json({
      success: false,
