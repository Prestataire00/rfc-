...
import {
  ListChecks, CheckCircle2, Circle, AlertCircle, Trash2,
...
/**
 * Panneau complet de gestion des tâches d'un projet.
 * Utilisé à la fois dans l'onglet "Tâches" de /projets/[id] (inline) et
 * dans la page dédiée /projets/[id]/taches (qui reste pour bookmarks).
 */
export function ProjetTachesPanel({ projetId }: { projetId: string }) {
  ...
  const createList = async () => {
    ...
    await fetch("/api/task-lists", {
      ...
      body: JSON.stringify({ nom: newListNom.trim(), couleur: newListCouleur, projetId }),
    });
    setNewListNom("");
  ...
  if (error) {
    return (
      <p className="text-sm text-red-500 p-4 border border-red-500/30 rounded-md bg-red-500/5">
