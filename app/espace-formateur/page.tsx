...
export default function EspaceFormateurPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Mon Espace Formateur</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="rounded-lg border bg-white p-5 flex items-center gap-4">
          <div className="rounded-full p-3 bg-blue-100"><CalendarDays className="h-6 w-6 text-blue-600" /></div>
          <div>
            <p className="text-sm text-gray-500">Sessions a venir</p>
            <p className="text-2xl font-bold text-gray-900">{aVenir.length}</p>
          </div>
        </div>
        <div className="rounded-lg border bg-white p-5 flex items-center gap-4">
          <div className="rounded-full p-3 bg-yellow-100"><Clock className="h-6 w-6 text-yellow-600" /></div>
          <div>
        ...
        <div className="rounded-lg border bg-white p-5 flex items-center gap-4">
          <div className="rounded-full p-3 bg-green-100"><CheckCircle className="h-6 w-6 text-green-600" /></div>
          <div>
        ...
        {aVenir.length === 0 ? (
          ...
          <div className="divide-y">
            {aVenir.slice(0, 10).map((s) => {
              ...
              return (
                <Link key={s.id} href={`/espace-formateur/sessions/${s.id}`} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50">
                  <div className="flex-shrink-0 text-center w-12">
                    <div className="text-lg font-bold text-blue-600">{new Date(s.dateDebut).getDate()}</div>
