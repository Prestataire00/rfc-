...
export async function GET() {
  try {
    ...
    console.error("Erreur GET inscriptions client:", err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("Erreur POST inscription client:", err);
    return NextResponse.json({ error: "Erreur lors de la demande d'inscription" }, { status: 500 });
  }
}
