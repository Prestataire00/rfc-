              ...
              {evaluations.map((ev) => (
                <tr key={ev.id} className="border-b last:border-0 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/sessions/${ev.session.id}`} className="text-blue-600 hover:underline">
                      {ev.session.formation.titre}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{EVALUATION_TYPES[ev.type as keyof typeof EVALUATION_TYPES]?.label || ev.type}</td>
                  <td className="px-4 py-3 text-gray-600 capitalize">{ev.cible}</td>
                  <td className="px-4 py-3 text-gray-700">{ev.contact ? `${ev.contact.prenom} ${ev.contact.nom}` : "—"}</td>
                  <td className="px-4 py-3">
                    {ev.noteGlobale ? (
